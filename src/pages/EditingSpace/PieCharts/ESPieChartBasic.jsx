...
import styles from "./ESPieChartBasic.module.css";

const ESPieChartBasic = () => {
  ...
  const [data, setData] = useState([44, 55, 13, 43, 22]);

  const [chartTitle, setChartTitle] = useState("Basic Pie Chart");
    ...
    try {
      ...
      const response = await fetch(
          ...
          : "http://localhost:3000/api/charts",
        {
          ...
          body: JSON.stringify({
            chart_type: "pie", // ✅ tipe pie chart
            title: chartTitle,
            categories: JSON.stringify(categories),
            data: JSON.stringify(data), // ✅ langsung array angka
            line_color: JSON.stringify(colors),
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (parsed.colors) {
        const c =
          typeof parsed.colors === "string"
            ? JSON.parse(parsed.colors)
            : parsed.colors;
        if (Array.isArray(c)) setColors(c);
      }
    } catch (err) {
...
export default ESPieChartBasic;
