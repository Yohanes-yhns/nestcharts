...
import styles from "./ESBarChartStacked.module.css";
import Navbar from "../../../components/Navbar";
import html2canvas from "html2canvas";

const ESBarChartStacked = () => {
  ...
  const [dataHigh, setDataHigh] = useState([44000, 55000, 41000, 67000, 22000]);
  const [dataLow, setDataLow] = useState([48000, 50000, 40000, 65000, 25000]);
  const [seriesNameHigh, setSeriesNameHigh] = useState("Input 1");
  const [seriesNameLow, setSeriesNameLow] = useState("Input 2");
  const [chartTitle, setChartTitle] = useState("Stacked Bar Chart");
  const [xAxisTitle, setXAxisTitle] = useState("Total");
  const [yAxisTitle, setYAxisTitle] = useState("Allocation");
  ...
  const chartOptions = {
    ...
    xaxis: {
      ...
      labels: {
        formatter: (val) => (val ? val / 1000 + "K" : "0K"),
      },
    },
    yaxis: {
      title: {
      ...
      labels: { style: { fontFamily, color: "#153448" } },
    },
  };
    ...
    try {
      ...
      const response = await fetch("http://localhost:3000/api/charts", {
        ...
        body: JSON.stringify({
          ...
          line_color: JSON.stringify([lineColor, lineColorLow]),
  ...
  const isValid =
    dataHigh.length === categories.length &&
    dataLow.length === categories.length &&
    dataHigh.every((d) => typeof d === "number" && !isNaN(d)) &&
    dataLow.every((d) => typeof d === "number" && !isNaN(d));
      ...
      <div className={styles.editorChartWrapper}>
        ...
        <div className={styles.chart} ref={chartPreviewRef}>
          {isValid ? (
            ...
            <p style={{ color: "red" }}>
              ❌ Jumlah data dan label harus sama dan valid.
            </p>
          )}
        </div>
      </div>
...
export default ESBarChartStacked;
