...
import styles from "./ESRadialChartBasic.module.css";
import Navbar from "../../../components/Navbar";
import html2canvas from "html2canvas";
import Swal from "sweetalert2"; // pastikan ini di atas
import { useNavigate } from "react-router-dom"; // untuk redirect jika perlu

const ESRadialChartBasic = () => {
  // MEMASUKAN DATA KEDALAM APEX *********************************

  const chartPreviewRef = useRef(null);

  const [categories, setCategories] = useState(["LABELS"]);
  const [data, setData] = useState([70]);
  const [chartTitle, setChartTitle] = useState("Basic Radial Chart");
  ...
  const handleSaveChart = async () => {
    const chartData = {
      ...
      x_axis_title: "", // tidak dipakai di radial
      y_axis_title: "", // tidak dipakai di radial
    };
    ...
    try {
      ...
      if (!res.ok) throw new Error("Gagal menyimpan chart");

      Swal.fire({
        icon: "success",
        title: "Berhasil",
        text: "Chart berhasil disimpan!",
      });

      // opsional, redirect ke saved charts
...
export default ESRadialChartBasic;
